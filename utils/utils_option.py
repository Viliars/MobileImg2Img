...
def save(opt):
    ...
    with open(dump_path, 'w') as dump_file:
        yaml.dump(opt, dump_file, indent=2)
...
def ordereddict_to_dict(opt):
    if isinstance(opt, dict):
        new_opt = dict()
        for key, sub_opt in opt.items():
            new_opt[key] = ordereddict_to_dict(sub_opt)
        return new_opt
    elif isinstance(opt, list):
        return [ordereddict_to_dict(sub_opt) for sub_opt in opt]
    else:
        return opt
